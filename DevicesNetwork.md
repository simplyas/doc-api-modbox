...
Este recurso permite aos equipamentos Modbox exibir um relógio personalizado na tela.</p>
<ul>
...
<hr>
<br>
<h2 id="lista-relógios"><strong>Lista Relógios Parâmetro  RecursoObservação
...
* **Parâmetros**
	...
	|--|--|--|
	| contract_hashObrigatórioHash do contrato |
	| user_token | ObrigatórioToken do usuário |</p>
</li>
<li>
...
--header 'Content-Type: application/json' \
  --data '{"
400{"error":"Verifique o JSON enviado."}400{"error":"Informe um hash correto."}403{"error":"Sem permissão ao recurso."}400{"error":"Verifique os parâmetros enviados."}500{"error":"Algo deu errado. Tente novamente."}curl --request POST \
--url 'http://{{api-url}}/requestClocks' \
--header 'Authorization: Basic bW9kYm94XYIuMDowRTk2QTRCNQ==' \
--header 'Content-Type: application/json' \
--data '{"contract_hash":"{{contract_hash}}", "user_token":"{{user_token}}"}'
 Parâmetro  RecursoObservação

contract_hash":"{{contract_hash}}", "user_token":"{{user_token}}"}'


<h2 id="novo| Parâmetro | Recurso | Observação |
|--|--|--|
...
<p>|Código| Resposta |
|--|--|
| 200 | code>`{"message":"Ajustes aplicados com sucesso!"}</co|
| 40 | cod>`{"error":"Informe uma hash correta."}</code>
` 
	| 400 | <code>`{"error":"Verifique o JSON enviado."}</code>` |
	| 400 | <code>`{"error":"Verifique os parâmetros enviados."}</code> |
| 403 | code>`{"error":"Sem permissão ao recurso."}</code> |
...
--data '{"
<br>
Recebe requisição contendo a hash do contrato, token e propriedades para atualizar um relógio. Parâmetro  RecursoObservação

contract_hash":"{{contract_hash}}", "user_token":"{{user_token}}","label":"{{label}}","background_color":"{{background_color}}","font_color":"{{font_color}}","transparent":"{{transparent}}","screen_area":"{{screen_area}}","screen_layer":"{{screen_layer}}","language":"{{language}}"}'
<br/>
<h2 id="atualiza-relógio"><strong>Atualiza Relógio</strong></h2>
<p>
**Atualiza Relógio**
----
 Recebe requisição contendo a hash do contrato, token e propriedades para atualizar um relógio.</p>
<ul>
<li>
<p><strong>URL</strong></p>
...
Bjb21tb25tYXJrXG4iLCJoaXN0b3J5IjpbLTgxNzc2OTc2OV19
